...
object GameOptions {
	...
	fun update() {
		if (tick % 5 == 0) {
			...
			if (Input.isKeyDown(38) && optionNum > 0) optionNum--
			if (Input.isKeyDown(40) && optionNum < 2) optionNum++
		}
		if (Input.isKeyDown(10)) {
			when (gameMode) {
				0 -> {
					StateManager.game = FFA
					FFA.init(numberOfEnemies, difficulty + 1)
				}
				1 -> {
					StateManager.game = SuppressUprising
					SuppressUprising.init(numberOfEnemies, difficulty + 1)
				}
				2 -> {
					StateManager.game = ZombieApocalypse
