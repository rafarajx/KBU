...
open class Entity {
	...
	fun <T: Nature> getNearestNature(arrayList: ArrayList<T>): vec2 {
		var max = Float.MAX_VALUE
		var target = p.copy()
		for (i in arrayList.indices) {
			val nature = arrayList[i]
	...
	fun <T: Building> getNearestBuilding(arrayList: ArrayList<T>): vec2 {
		var max = Float.MAX_VALUE
		var target = p.copy()
		for (i in arrayList.indices) {
			val building = arrayList[i]
			val delta = building.p - p
			val d = hypot(delta.x, delta.y)
			if (max > d) {
				max = d
